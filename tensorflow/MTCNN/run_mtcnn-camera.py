...
def detect_face(img, threshold = [0.405, 0.8473], factor=0.709):
    ...
    # Open and allocate graph for pgraph 
    device.OpenDevice()
    pgraph = device.AllocateGraph(pgraphfile)
    ...
    if numbox > 0:
        ...
        # deallocate first graph file and close the device here
        pgraph.DeallocateGraph()
        device.CloseDevice()
        # re-open the device
        device.OpenDevice()
        # allocate the second graph file
        ...
        # deallcoate second graph file and close the device here
...
def main():
    ...
    # Main loop: Capture live stream and send frames to NCS
    while( True ):
       ret, frame = camera.read()
       image = pre_process_image(frame)
       infer_image(image, frame)
...
def infer_image(image, frame):
    rects = detect_face(image)
    ...
    cv2.imshow('MTCNN NCS Demo', image)
